...
describe("sectionHtml rowBudget", () => {
  const section = {
    name: "NBA",
    prefix: "sensor.nba_",
    limit: 10,
  ...
  it("limits rendered rows when rowBudget is smaller than limit", () => {
    const states = Object.fromEntries(
    ...
    const el = doc(sectionHtml(section, states, Object.keys(states), {}, new Map(), 3));
    expect(el.querySelectorAll(".game-row").length).toBe(3);
  });

  it("respects user limit when it is smaller than rowBudget", () => {
    const states = Object.fromEntries(
      Array.from({ length: 10 }, (_, i) => [
        `sensor.nba_team${i}`,
        makeState("PRE", { ...baseAttrs, team_name: `Team${i}`, team_record: `${i}-10` }),
    ...
    const el = doc(
      sectionHtml({ ...section, limit: 2 }, states, Object.keys(states), {}, new Map(), 5)
    );
    expect(el.querySelectorAll(".game-row").length).toBe(2);
  });

  it("renders no rows when rowBudget is 0", () => {
    const states = { "sensor.nba_lal": makeState("PRE", baseAttrs) };
    const el = doc(sectionHtml(section, states, Object.keys(states), {}, new Map(), 0));
    expect(el.querySelector(".section-header")).toBeNull();
  });

  it("uses normal limit when rowBudget is undefined", () => {
    const states = Object.fromEntries(
      Array.from({ length: 10 }, (_, i) => [
        `sensor.nba_team${i}`,
        makeState("PRE", { ...baseAttrs, team_name: `Team${i}`, team_record: `${i}-10` }),
    ...
    const el = doc(
      sectionHtml({ ...section, limit: 5 }, states, Object.keys(states), {}, new Map(), undefined)
    );
    expect(el.querySelectorAll(".game-row").length).toBe(5);
  });
});
