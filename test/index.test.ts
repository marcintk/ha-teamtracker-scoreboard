...
describe("SportScoreboardCard", () => {
  ...
  describe("_render", () => {
    ...
    it("distributes row budget proportionally across multiple sections", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 10 },
          { ...nbaSection, prefix: "sensor.nhl_", limit: 5 },
        ],
        height: "300px",
      };
      const states = Object.fromEntries([
        ...Array.from({ length: 20 }, (_, i) => [
          `sensor.nba_team${i}`,
          makeState("PRE", { ...baseAttrs, team_name: `NBATeam${i}` }),
        ]),
        ...Array.from({ length: 10 }, (_, i) => [
          `sensor.nhl_team${i}`,
          makeState("PRE", { ...baseAttrs, team_name: `NHLTeam${i}` }),
        ]),
      ]);
      card._hass = makeHass(states);
      card._render();
      const rows = card.shadowRoot?.querySelectorAll(".game-row");
      expect(rows && rows.length).toBeLessThanOrEqual(10);
    });

    it("allocates remaining budget rows to sections that can use them", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 10 },
          { ...nbaSection, prefix: "sensor.nhl_", limit: 10 },
          { ...nbaSection, prefix: "sensor.mlb_", limit: 10 },
        ],
        height: "300px",
      };
      const states = Object.fromEntries([
        ...
        ...Array.from({ length: 15 }, (_, i) => [
          `sensor.mlb_team${i}`,
          makeState("PRE", { ...baseAttrs, team_name: `MLBTeam${i}` }),
        ]),
      ]);
      card._hass = makeHass(states);
      card._render();
    ...
    it("distributes remainder when total budget does not divide evenly", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 5 },
      ...
      expect(rows && rows.length).toBeLessThanOrEqual(6);
    });

    it("respects section limits when allocating remainder budget", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 2 },
    ...
    it("handles sections with no limit specified during budget distribution", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { name: "NBA", prefix: "sensor.nba_" },
          { name: "NHL", prefix: "sensor.nhl_" },
        ],
        height: "300px",
      };
      const states = Object.fromEntries([
    ...
    it("stops allocating remainder when all sections hit their limit", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 1 },
          { ...nbaSection, prefix: "sensor.nhl_", limit: 1 },
        ],
        height: "200px",
      };
      const states = Object.fromEntries([
    ...
    it("stops remainder distribution early when remainder depletes", () => {
      const card = makeCard();
      card._config = {
        sections: [
          { ...nbaSection, limit: 10 },
          { ...nbaSection, prefix: "sensor.nhl_", limit: 10 },
          { ...nbaSection, prefix: "sensor.mlb_", limit: 10 },
          { ...nbaSection, prefix: "sensor.nfl_", limit: 10 },
        ],
        height: "180px",
      };
      const states = Object.fromEntries([
        ...
        ...Array.from({ length: 10 }, (_, i) => [
          `sensor.nfl_team${i}`,
          makeState("PRE", { ...baseAttrs, team_name: `NFLTeam${i}` }),
        ]),
      ]);
      card._hass = makeHass(states);
      card._render();
      const rows = card.shadowRoot?.querySelectorAll(".game-row");
      expect(rows && rows.length).toBeGreaterThan(0);
      expect(rows && rows.length).toBeLessThanOrEqual(5);
    });

    it("exits remainder loop when sections exhausted before remainder", () => {
      const card = makeCard();
      card._config = {
        sections: [{ ...nbaSection, limit: 3 }],
        height: "200px",
      };
      const states = Object.fromEntries(
        Array.from({ length: 10 }, (_, i) => [
      ...
      card._render();
      const rows = card.shadowRoot?.querySelectorAll(".game-row");
      expect(rows && rows.length).toBeLessThanOrEqual(3);
    });

    it("handles height too small for any rows gracefully", () => {
      const card = makeCard();
      card._config = {
        sections: [{ ...nbaSection, limit: 10 }],
        height: "50px",
      };
      const states = Object.fromEntries(
        Array.from({ length: 5 }, (_, i) => [
