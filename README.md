...
| `show_position` | boolean | `true`   | **Card-level.** `false` removes the position gutter from every row entirely. Use when no section is a `ranking` view; per-section control is the `show_position` [Section](#section) field                                            |
...
| `show_position` | boolean | `true`          | Show this section's position numbers. Only applies in the `ranking` view — a `schedule` section's gutter is always blank. `false` blanks the numbers but keeps the gutter drawn, so mixed sections stay row-aligned           |
